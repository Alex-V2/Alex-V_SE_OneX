/*
 * arch/arm/mach-tegra/board-endeavortd-power.c
 *
...
TPS_PDATA_INIT(smps3, common, 600, 2100, 0, 0, 0, 0, 1225, 1, 1, 0, 0, 0);
...
TPS_PDATA_INIT(ldo7, common, 1000, 3300, tps80031_rails(VIO), 0, 0, 1, 1200, 1, 1, 0, PWR_REQ_INPUT_PREQ1, 0);
...
static int __init endeavortd_fixed_regulator_init(void)
{
	int i;
...
static int __init endeavortd_gpio_regulator_init(void)
{
	int i, j;
...
static int __init endeavortd_regulators_fixed_gpio_init(void)
{
	int ret;
	...
	ret = endeavortd_fixed_regulator_init();
	if (ret)
		return ret;

	ret = endeavortd_gpio_regulator_init();
	return ret;
}
subsys_initcall_sync(endeavortd_regulators_fixed_gpio_init);

static struct i2c_board_info __initdata endeavortd_regulators[] = {
	{
...
#ifdef CONFIG_PM
static void endeavortd_suspend_work(void)
{
...
static void endeavortd_resume_work(void)
{
...
static int __init endeavortd_gpio_rtc_init(void)
{
	int ret;
...
#ifdef CONFIG_PM
	tps_platform.suspend_work = endeavortd_suspend_work;
	tps_platform.resume_work = endeavortd_resume_work;
#endif
	return 0;
...
void __init endeavortd_tsensor_init(void)
{
	tegra3_tsensor_init(NULL);
}

int __init endeavortd_regulator_init(void)
{
	...
	endeavortd_gpio_rtc_init();

	// TODO: test HW phase before assign subdevs
	tps_platform.num_subdevs = ARRAY_SIZE(tps80031_devs);
	tps_platform.subdevs = tps80031_devs;

	i2c_register_board_info(4, endeavortd_regulators,
		    ARRAY_SIZE(endeavortd_regulators));

	endeavortd_tsensor_init();
	return 0;
}

static void endeavortd_board_suspend(int lp_state, enum suspend_stage stg)
{
	if ((lp_state == TEGRA_SUSPEND_LP1) && (stg == TEGRA_SUSPEND_BEFORE_CPU))
		tegra_console_uart_suspend();
}

static void endeavortd_board_resume(int lp_state, enum resume_stage stg)
{
	if ((lp_state == TEGRA_SUSPEND_LP1) && (stg == TEGRA_RESUME_AFTER_CPU))
		tegra_console_uart_resume();
}

static struct tegra_suspend_platform_data endeavortd_suspend_data = {
	.cpu_timer	= 600,
	.cpu_off_timer	= 200,
...
int __init endeavortd_suspend_init(void)
{
	tegra_init_suspend(&endeavortd_suspend_data);
	return 0;
}

#ifdef CONFIG_TEGRA_EDP_LIMITS
int __init endeavortd_edp_init(void)
{
